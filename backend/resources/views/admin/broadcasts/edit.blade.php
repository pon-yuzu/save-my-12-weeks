...
<div class="admin-card">
    ...
    <form method="POST" action="{{ route('admin.broadcasts.update', $broadcast) }}" id="broadcastForm">
        @csrf
        @method('PUT')

        <div class="form-group">
            <label class="form-label">件名</label>
            <input type="text" name="subject" class="form-input" value="{{ old('subject', $broadcast->subject) }}" required>
        </div>

        <div class="form-group">
            <label class="form-label">本文</label>
            <textarea name="body" class="form-textarea" style="min-height: 250px;" required>{{ old('body', $broadcast->body) }}</textarea>
        ...
        <div class="form-group">
            ...
            <div style="display: flex; flex-direction: column; gap: 12px;">
                <label style="display: flex; align-items: center; gap: 8px;">
                    <input type="radio" name="target_type" value="all" {{ old('target_type', $broadcast->target_type) === 'all' ? 'checked' : '' }} onchange="toggleTargetOptions()">
                    <span>全員に送信</span>
                </label>

                <label style="display: flex; align-items: center; gap: 8px;">
                    <input type="radio" name="target_type" value="filtered" {{ old('target_type', $broadcast->target_type) === 'filtered' ? 'checked' : '' }} onchange="toggleTargetOptions()">
                    <span>条件でフィルター</span>
                </label>
                ...
                <div id="filterOptions" style="margin-left: 24px; display: none;">
                    <div style="display: flex; gap: 16px; align-items: center;">
                        <label>Day
                            <input type="number" name="day_min" class="form-input" style="width: 80px;" placeholder="最小" value="{{ old('day_min', $broadcast->target_filter['day_min'] ?? '') }}">
                        </label>
                        <span>〜</span>
                        <label>
                            <input type="number" name="day_max" class="form-input" style="width: 80px;" placeholder="最大" value="{{ old('day_max', $broadcast->target_filter['day_max'] ?? '') }}">
                        </label>
                    </div>
                </div>

                <label style="display: flex; align-items: center; gap: 8px;">
                    <input type="radio" name="target_type" value="individual" {{ old('target_type', $broadcast->target_type) === 'individual' ? 'checked' : '' }} onchange="toggleTargetOptions()">
                    <span>個別に選択</span>
                </label>
                ...
                <div id="individualOptions" style="margin-left: 24px; display: none;">
                    <select name="recipient_ids[]" class="form-select" multiple style="height: 200px;">
                        @foreach($subscribers as $subscriber)
                            <option value="{{ $subscriber->id }}" {{ in_array($subscriber->id, old('recipient_ids', $broadcast->recipient_ids ?? [])) ? 'selected' : '' }}>
        ...
        <div class="form-group">
            ...
            <div style="display: flex; flex-direction: column; gap: 12px;">
                <label style="display: flex; align-items: center; gap: 8px;">
                    <input type="radio" name="send_timing" value="now" onchange="toggleScheduleOptions()">
                    <span>今すぐ送信</span>
                </label>

                <label style="display: flex; align-items: center; gap: 8px;">
                    <input type="radio" name="send_timing" value="scheduled" {{ $broadcast->scheduled_at ? 'checked' : '' }} onchange="toggleScheduleOptions()">
                    <span>予約配信</span>
                </label>

                <div id="scheduleOptions" style="margin-left: 24px; display: none;">
                    <input type="datetime-local" name="scheduled_at" class="form-input" value="{{ old('scheduled_at', $broadcast->scheduled_at?->format('Y-m-d\TH:i')) }}">
                </div>

                <label style="display: flex; align-items: center; gap: 8px;">
                    <input type="radio" name="send_timing" value="draft" {{ !$broadcast->scheduled_at ? 'checked' : '' }} onchange="toggleScheduleOptions()">
                    <span>下書き保存</span>
                </label>
            </div>
        </div>
        ...
        <div style="display: flex; gap: 12px;">
            <button type="submit" class="btn btn-primary" onclick="setSendNow()">保存</button>
        </div>
    </form>
</div>
